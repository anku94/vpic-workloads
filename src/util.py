...
class VPICReader:
    ...
    def __init__(self, path: str, num_ranks=None) -> None:
        ...
        self.timesteps = sorted(self._find_all_ts(), key=lambda x: int(x.name[2:]))
    ...
    def _find_all_ts(self):
        ...
        timesteps = list(
            filter(lambda x: re.match("T\.\d+", x.name) and x.is_dir(), candidates)
        )
        return timesteps
    ...
    def read_a_rank(
        self, timestep: int, rank: int, ftype: str = "eparticle"
    ) -> List[str]:
    ...
    @staticmethod
    def sample_a_rank(
        fpath: str, intvl_cnt: int = 10, intvl_samples: int = 2
    ) -> List[float]:
        print(fpath)
    ...
    def sample_a_rank_unpack(args: Tuple) -> List[float]:
        return VPICReader.sample_a_rank(
            args[0], intvl_cnt=args[1], intvl_samples=args[2]
        )

    def rank_fpath(self, timestep: int, rank: int, ftype: str = "eparticle") -> str:
        ts_str = re.findall("\d+", self.timesteps[timestep].name)[0]
        rank_fname = self.timesteps[timestep] / ("%s.%s.%s" % (ftype, ts_str, rank))
        return rank_fname

    def read_sample(self, timestep: int) -> np.array(float):
        ts_str = self.timesteps[timestep]
        fpath = ts_str / "sample.npy"
        samples = np.load(fpath)
        return samples

    def sample_hist(self, timestep: int) -> np.array(float):
        ts_str = self.timesteps[timestep]
        fpath = ts_str / "hist"
    ...
    def sample_global(
        self, timestep: int, ftype: str = "eparticle", load_cached=False
    ) -> np.array(float):
        num_ranks = self.get_num_ranks()
        rank_paths = [
            self.rank_fpath(timestep, rank, ftype) for rank in range(num_ranks)
        ]
        INTVL_COUNT = 100
        INTVL_SAMPLES = 100
    ...
    def __init__(
        ...
        if bin_weights is not None and bin_edges is not None:
            if isinstance(bin_weights, Iterable):
                ...
                raise Exception(
                    "Invalid constructor parameters: %s" % (type(bin_weights),)
                )
        ...
        for idx, item in enumerate(rb_items):
            ...
            if bp_bin_val != prev_bin_val:
                ...
                for rank in active_ranks:
                    ...
                    rank_contrib = (
                        rank_bin_widths[rank] * rank_left_range * 1.0 / rank_total_range
                    )
